...
function days({ daysOfWeek, userId, cellRefs, userIndex, handleMouseDown, handleResizeMouseDown, handleAddSimpleEvent, handleAddDetailsEvent, activeEventId, handleShowEventDetails, events }: props) {
  const ms_events = events.filter((event) => event.event_type==="outlook_event" );
  const holiday_events = events.filter((event) => event.event_type ==="holiday_event");
  const normal_events = events.filter((event) => event.event_type === "work_event" );
  ...
  return daysOfWeek.map( (day, dayIndex) => {
    ...
    const holidayEvents = holiday_events.filter((ev) =>  dayjs(ev.start).isBefore(day.endOf("day")) && dayjs(ev.end).isAfter(day.startOf("day")));
    ...
    return (
      ...
      >
        ...
        <OutlookEvents outlookEvents={msEvents} day={day}/>


        {/* Render ms_events directly (like FreeTime, no column handling) */}
        <HolidayEvents holidayEvents={holidayEvents} day={day}/>
